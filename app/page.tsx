...
export default function ComunidadPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFD700] text-black selection:bg-black selection:text-[#FFD700] font-sans flex flex-col justify-between">
        ...
        <header className="bg-white border-b-4 border-black py-4 px-6 sticky top-0 z-50">
          <div className="max-w-6xl mx-auto flex justify-between items-center">
            <div className="flex items-center gap-3">
              <h1 className="font-display text-2xl uppercase tracking-tight text-shadow-hard shrink-0">
                🐣 TEAM POLLITO
              </h1>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 mt-8">
          ...
          {session && !loadingStatus && statusInfo.status === 'approved' ? (
            ...
            <div className="max-w-4xl mx-auto space-y-8">
              <motion.div
                whileHover={{ y: -4 }}
                className="bg-white border-4 border-black p-6 sm:p-8 rounded-[2rem] shadow-[8px_8px_0_0_#000] text-center space-y-6 relative overflow-hidden"
              >
                <div className="absolute -top-10 -right-10 w-40 h-40 bg-gradient-to-br from-yellow-300 to-amber-400 opacity-20 rounded-full blur-2xl pointer-events-none" />

                <div className="w-20 h-20 bg-gradient-to-tr from-yellow-400 to-amber-300 border-4 border-black rounded-3xl flex items-center justify-center mx-auto text-4xl shadow-[4px_4px_0_0_#000] animate-bounce">
                  👑
                </div>

                <div className="space-y-2">
                  <h3 className="font-display text-3xl uppercase tracking-tight">¡Bienvenido, Miembro Oficial!</h3>
                  <p className="font-comic text-sm sm:text-base font-bold text-gray-600 max-w-lg mx-auto leading-relaxed">
                    Ya formás parte del selecto **Team Pollito**. Tenés acceso completo a la Consola VIP interactiva para reproducir sonidos y enviar mensajes en directo.
                  </p>
                </div>

                {/* PROFILE CARD */}
                <div className="bg-yellow-50/50 border-3 border-black p-4 rounded-2xl max-w-sm mx-auto flex items-center gap-4 shadow-[4px_4px_0_0_#000]">
                  ...
                  <div className="text-left min-w-0">
                    ...
                    <p className="font-display text-sm uppercase text-black leading-none truncate mt-1">
                      @{statusInfo.roblox_user}
                    </p>
                    <p className="font-comic text-[10px] text-gray-500 font-bold mt-0.5 truncate">
                ...
                <div className="pt-2">
                  <Link
                    href="/console"
                    className="inline-flex items-center justify-center gap-2 w-full max-w-md py-4 bg-[#FFD700] hover:bg-yellow-300 text-black font-display text-sm uppercase font-black border-3 border-black rounded-xl shadow-[4px_4px_0_0_#000] active:translate-y-0.5 active:shadow-none transition-all decoration-transparent"
                  >
                    ✓ ENTRAR A LA CONSOLA VIP
                  </Link>
                </div>
              </motion.div>

              {/* EXPANDABLE/ACCORDEON SECTIONS */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">

                {/* MEMBERS LIST */}
                <div className="bg-white border-4 border-black rounded-3xl p-6 shadow-[6px_6px_0_0_#000] flex flex-col h-[460px]">
                  <h3 className="font-display text-lg uppercase mb-4 border-b-2 border-black pb-2 flex items-center gap-2 shrink-0">
                  ...
                  {loadingMembers ? (
                    ...
                    <div className="flex-1 flex items-center justify-center bg-yellow-50 rounded-2xl border-2 border-dashed border-gray-300">
                      <p className="font-comic text-xs font-bold text-gray-400 uppercase">Sin miembros registrados</p>
                    </div>
                  ) : (
                    <div className="flex-1 overflow-y-auto pr-1.5 scrollbar-thin">
                      <div className="grid grid-cols-2 gap-4 p-1">
                        {sortedMembers.map((member) => {
                          const isStreamer = member.roblox_user.toLowerCase().includes('milumon');
                          return (
                            <motion.div
                              key={member.roblox_user}
                              whileHover={{ scale: 1.02 }}
                              className={`border-3 border-black p-3 rounded-xl text-center flex flex-col items-center justify-between shadow-[2px_2px_0_0_rgba(0,0,0,1)] relative ${isStreamer
                                  ? 'bg-amber-100 border-amber-500 shadow-[2px_2px_0_0_#f97316]'
                                  : 'bg-yellow-50/30'
                                }`}
                            >
                              {isStreamer && (
                                <span className="absolute -top-2 -right-1 bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-black text-[6px] uppercase tracking-wider px-1.5 py-0.5 border border-black rounded shadow-[1px_1px_0_0_#000] rotate-6 z-10">
                                  Streamer
                                </span>
                              )}
                              {member.roblox_avatar_url ? (
                                <img
                                  ...
                                  className={`w-12 h-12 rounded-full border-2 object-cover mb-1.5 ${isStreamer ? 'border-amber-500 ring-2 ring-amber-300' : 'border-black'
                                    }`}
                                />
                              ) : (
                                <div className={`w-12 h-12 rounded-full border-2 flex items-center justify-center text-lg mb-1.5 ${isStreamer ? 'border-amber-500 bg-amber-200' : 'border-black bg-yellow-300'
                                  }`}>
                                  🐣
                                </div>
                              )}
                              <p className="font-display text-xs uppercase text-black leading-none truncate max-w-full">
                ...
                {/* RULES CARD COLLAPSED */}
                <div className="bg-white border-4 border-black rounded-3xl p-6 shadow-[6px_6px_0_0_#000] flex flex-col h-[460px]">
                  <h3 className="font-display text-lg uppercase mb-4 border-b-2 border-black pb-2 flex items-center gap-2 shrink-0">
                    📋 REGLAS GENERALES
                  </h3>
                  <div className="flex-1 overflow-y-auto pr-1 scrollbar-thin text-left space-y-4">
                    <div className="border-l-4 border-black pl-3 py-1 bg-yellow-50/40 rounded-r-xl">
                      <h4 className="font-display text-xs uppercase text-amber-800">1. Respeto Mutuo 🐤</h4>
                      <p className="font-comic text-[11px] font-bold text-gray-600 mt-1">Respetar a Milumon y a todos los miembros de la comunidad en chats y streams.</p>
                    </div>
                    <div className="border-l-4 border-black pl-3 py-1 bg-yellow-50/40 rounded-r-xl">
                      <h4 className="font-display text-xs uppercase text-amber-800">2. Uso de la Consola 🐤</h4>
                      <p className="font-comic text-[11px] font-bold text-gray-600 mt-1">No spamear audios o TTS en vivo. El abuso resultará en ban permanente.</p>
                    </div>
                    <div className="border-l-4 border-black pl-3 py-1 bg-yellow-50/40 rounded-r-xl">
                      <h4 className="font-display text-xs uppercase text-amber-800">3. Puntualidad 🐤</h4>
                      <p className="font-comic text-[11px] font-bold text-gray-600 mt-1">Si agendás entrevista, estate conectado en Roblox y en el stream en ese horario.</p>
                    </div>
                    <div className="border-l-4 border-black pl-3 py-1 bg-yellow-50/40 rounded-r-xl">
                      <h4 className="font-display text-xs uppercase text-amber-800">4. Tag Oficial 🐣</h4>
                      <p className="font-comic text-[11px] font-bold text-gray-600 mt-1">Llevar con orgullo el prefijo 🐣 en Roblox una vez aprobado.</p>
                    </div>
                  </div>
                </div>
            ...
            /* ================= STANDARD VISITOR / CANDIDATE VIEW ================= */
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8 space-y-8">
                ...
                <div className="bg-white border-4 border-black rounded-3xl p-6 brutalist-shadow flex flex-col">
                  <h3 className="font-display text-xl uppercase mb-4 border-b-2 border-black pb-2 flex items-center gap-2 shrink-0">
                    🌟 MIEMBROS OFICIALES ({members.length})
                  </h3>
                  ...
                  {loadingMembers ? (
                    ...
                    <div className="max-h-[340px] overflow-y-auto pr-1.5 scrollbar-thin">
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-1">
                        {sortedMembers.map((member) => {
                          ...
                          return (
                            ...
                            >
                              {isStreamer && (
                                <span className="absolute -top-2.5 -right-1 bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-black text-[7px] uppercase tracking-wider px-1.5 py-0.5 border border-black rounded shadow-[1px_1px_0_0_#000] rotate-6 z-10">
                                  👑 Streamer
                                </span>
                              )}
                              {member.roblox_avatar_url ? (
                                <img
                                  src={member.roblox_avatar_url}
                                  alt={member.roblox_display_name}
                                  className={`w-16 h-16 rounded-full border-2 object-cover mb-2 ${isStreamer ? 'border-amber-500 ring-2 ring-amber-300' : 'border-black'
                                    }`}
                                />
                              ) : (
                                <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center text-xl mb-2 ${isStreamer ? 'border-amber-500 bg-amber-200' : 'border-black bg-yellow-300'
                                  }`}>
                                  🐣
                                </div>
                              )}
                ...
                {session && (
                  <div className="sticky top-24 space-y-6">
                    ...
                    {!loadingStatus && statusInfo.status === 'pending' && (
                      <div className="bg-white border-4 border-black rounded-3xl p-6 brutalist-shadow space-y-4 text-center">
                          ...
                          <form onSubmit={handleBook} className="space-y-4">
                            ...
                            <div className="flex border-2 border-black rounded-xl overflow-hidden text-center shrink-0">
                              <button
                                ...
                                className={`flex-grow py-2 font-comic text-[11px] font-black uppercase transition-all ${!isReturning ? 'bg-yellow-400 text-black' : 'bg-white text-gray-500'}`}
                              >
                                Postulante Nuevo
                              </button>
                              <button
                                ...
                                className={`flex-grow py-2 font-comic text-[11px] font-black border-l-2 border-black uppercase transition-all ${isReturning ? 'bg-[#ea580c] text-white' : 'bg-white text-gray-500'}`}
                              >
                                Pedido Re-ingreso
                              </button>
                            </div>
      ...
      {/* FOOTER BRUTALISTA */}
      <footer className="mt-16 bg-black text-[#FFD700] border-t-4 border-black py-8 px-6 text-center select-none shrink-0">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 font-comic text-xs font-black">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <a href="https://www.tiktok.com/@milumon_gamer" target="_blank" rel="noopener noreferrer" className="hover:underline">
              📱 TikTok de Milumon
            </a>
            <Link href="/awards" className="text-yellow-400 hover:text-white underline decoration-2 font-display uppercase tracking-wider">
