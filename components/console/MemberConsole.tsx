                    ...
                    <div className="bg-[#2b2d31] border border-neutral-700/60 rounded-2xl p-5 shadow-[0_4px_12px_rgba(0,0,0,.25)]">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {([
                          ...
                          <Link
                            key={tab.id}
                            href={panelMode ? `/panel/sonidos?tipo=${tab.id}` : '/console'}
                            aria-current={displayedSoundType === tab.id ? 'page' : undefined}
                            onClick={(event) => {
                              soundManager.playPop();
                              if (!panelMode) {
                                event.preventDefault();
                                setSoundboardSubTab(tab.id);
                              }
                            }}
                            className={`px-3 py-1.5 rounded-full text-[11px] font-display font-semibold border transition-all cursor-pointer ${
                              displayedSoundType === tab.id
                        ...
                        {loadingSounds ? (
                          ...
                          (() => {
                            const filteredSounds = sounds.filter(s => {
                              if (displayedSoundType === 'audios') return !s.media_type || s.media_type === 'audio';
                              if (displayedSoundType === 'multimedia') return s.media_type === 'image_audio' || s.media_type === 'image';
                              if (displayedSoundType === 'videos') return s.media_type === 'video';
                              return true;
                            });
                            ...
                            if (filteredSounds.length === 0) {
                              return (
                                <div className="py-12 text-center text-xs font-bold text-gray-500 border border-dashed border-[#FFC200]/45 rounded-2xl bg-black/20">
                                  {displayedSoundType === 'audios' && 'No hay audios disponibles en este momento.'}
                                  {displayedSoundType === 'multimedia' && 'No hay imágenes disponibles en este momento.'}
                                  {displayedSoundType === 'videos' && 'No hay videos disponibles en este momento.'}
                                </div>
                              );
                            }
