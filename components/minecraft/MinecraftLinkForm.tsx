      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-8 sm:py-16">
        <div className="mb-8 flex items-center gap-3"><span className="text-4xl">🐣</span><div><p className="font-display text-sm font-bold uppercase tracking-[0.3em] text-[#D4A000]">Minecraft · Team Pollito</p><h1 className="font-display text-4xl font-black uppercase leading-[0.92] tracking-tight text-[#2D3139] sm:text-5xl">Vincula tu cuenta</h1></div></div>
          ...
          <>
            ...
            {step === 1 && <StepOne edition={edition} setEdition={(value) => { formHasChanges.current = true; setEdition(value); setUsername(accounts.find((item) => item.edition === value)?.username ?? ''); }} username={username} setUsername={(value) => { formHasChanges.current = true; setUsername(value); }} existingAccount={accounts.find((item) => item.edition === edition) ?? null} onNext={() => { setMessage(null); setStep(2); }} />}
...
function StepOne({ edition, setEdition, username, setUsername, existingAccount, onNext }: Readonly<{ edition: 'java' | 'bedrock'; setEdition: (value: 'java' | 'bedrock') => void; username: string; setUsername: (value: string) => void; existingAccount: Account | null; onNext: () => void }>) {
  return <Card title="Elige cómo juegas" icon="🎮"><p className="text-sm font-medium leading-relaxed text-[#64748B]">Puedes vincular una cuenta de cada tipo. Elige una ahora y después podrás volver para agregar la otra.</p><div className="mt-5 grid gap-3 sm:grid-cols-2" role="radiogroup" aria-label="Edición de Minecraft"><EditionOption selected={edition === 'java'} onSelect={() => setEdition('java')} title="Java" subtitle="Computadora / PC" description="Para Windows, Mac o Linux." icon="🖥️" /><EditionOption selected={edition === 'bedrock'} onSelect={() => setEdition('bedrock')} title="Bedrock" subtitle="Celular o tablet" description="La versión de teléfonos y tabletas." icon="📱" /></div><p className="mt-5 text-sm font-medium text-[#64748B]">Escribe el nombre que usas en Minecraft. Las mayúsculas no importan.</p><label className="mt-4 block text-sm font-bold text-[#45413A]">Usuario de Minecraft<input value={username} onChange={(event) => setUsername(event.target.value)} required maxLength={32} className="mt-2 w-full rounded-xl border border-[#E8DFC5] bg-[#FFFDF5] px-4 py-3 text-[#2D3139]" placeholder={edition === 'java' ? 'Ejemplo: camitupro1000' : 'Ejemplo: Cami3825'} /></label>{existingAccount && <p className="mt-3 rounded-xl bg-[#FFF7DC] p-3 text-xs font-bold text-[#7A6330]">Ya tienes una solicitud para {existingAccount.username} en {edition === 'java' ? 'Java' : 'Bedrock'}. Si continúas, generarás un código nuevo.</p>}<button type="button" onClick={onNext} disabled={!username.trim()} className="mt-6 w-full rounded-xl bg-[#FFD500] px-5 py-3 font-black text-black disabled:cursor-not-allowed disabled:opacity-50">Continuar ✨</button></Card>;
}

function EditionOption({ selected, onSelect, title, subtitle, description, icon }: Readonly<{ selected: boolean; onSelect: () => void; title: string; subtitle: string; description: string; icon: string }>) {
  return <label className={`relative flex cursor-pointer gap-3 rounded-2xl border-2 p-4 transition ${selected ? 'border-[#D4A000] bg-[#FFF7DC] shadow-[4px_4px_0_#FFD500]' : 'border-[#E8DFC5] bg-[#FFFDF5] hover:border-[#FFD500]'}`}><input type="radio" name="minecraft-edition" value={title.toLowerCase()} checked={selected} onChange={onSelect} className="sr-only" /><span className="text-3xl" aria-hidden>{icon}</span><span><span className="flex items-center gap-2 font-black text-[#2D3139]">{title}{selected && <span className="text-emerald-600" aria-label="Seleccionado">✓</span>}</span><span className="mt-1 block text-sm font-black text-[#8B6B00]">{subtitle}</span><span className="mt-1 block text-xs font-medium leading-relaxed text-[#64748B]">{description}</span></span></label>;
}
