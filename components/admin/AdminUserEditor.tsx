...
export function AdminUserEditor({ userId, onSaved }: { userId: string; onSaved?: () => void }) {
  ...
  if (usersError) {
    return <p className="rounded-xl border border-red-500/20 bg-red-950/40 p-4 text-sm text-red-400">{usersError}</p>;
  }
  if (!user) {
    return (
      <div className="rounded-2xl border border-neutral-700/60 bg-[#2b2d31] p-6 text-center">
        ...
        <Link href="/admin/usuarios" className="mt-4 inline-flex rounded-xl bg-[#FFC200] px-4 py-2 text-sm font-bold text-black">Volver a Usuarios</Link>
      </div>
    );
  }
...
function LoadedAdminUserEditor({
  ...
  return (
    <article className="flex max-h-[78vh] flex-col text-gray-200">
      ...
      <div className="mb-5 flex shrink-0 border-b border-neutral-700/60">
        {([['profile', 'Perfil'], ['votes', 'Premios y votos'], ['permissions', 'Permisos']] as const).map(([key, label]) => (
          <button
            ...
            className={`flex-1 cursor-pointer border-b-2 pb-2 text-[10px] font-bold uppercase tracking-wider ${tab === key ? 'border-[#FFC200] text-white' : 'border-transparent text-gray-500'}`}
          >
            {label}
          </button>
        ))}
      ...
      {editorError && <p className="mb-3 rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-xs font-semibold text-red-400">{editorError}</p>}
      {message && <p className="mb-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs font-semibold text-emerald-400">{message}</p>}
      ...
      {tab === 'profile' && (
        <form onSubmit={saveProfile} className="flex flex-1 flex-col gap-4 overflow-y-auto pr-1">
          <label className="space-y-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-400">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="min-w-0 flex-1 rounded-xl border border-neutral-700/60 bg-[#171A20] px-3.5 py-2.5 text-xs normal-case tracking-normal text-white outline-none focus:border-[#FFC200]"
              />
              <button type="button" onClick={() => void verifyRoblox()} disabled={validating || !form.robloxUsername.trim()} className="cursor-pointer rounded-xl bg-[#2b2d31] px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">
          ...
          {form.linkStatus === 'rejected' && (
            <label className="space-y-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-400">
              Motivo del rechazo
              <textarea value={form.rejectionReason} onChange={(event) => setForm({ ...form, rejectionReason: event.target.value })} rows={2} className="w-full resize-none rounded-xl border border-neutral-700/60 bg-[#171A20] px-3.5 py-2.5 text-xs normal-case tracking-normal text-white outline-none" />
            </label>
          )}
          <button type="submit" disabled={saving} className="mt-auto flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#FFC200] py-3 font-display text-xs font-semibold text-black disabled:opacity-50">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 space-y-2 overflow-y-auto pr-1">
            {permissions.map(([clientKey, , label]) => (
              <label key={clientKey} className="flex cursor-pointer items-center justify-between rounded-xl border border-neutral-700/60 bg-[#171A20] p-3 text-xs font-semibold text-white">
                {label}
          ...
          <button type="button" onClick={() => void savePermissions()} disabled={saving} className="mt-4 flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#FFC200] py-3 font-display text-xs font-semibold text-black disabled:opacity-50">
...
export function AdminUserEditorPage({ userId }: { userId: string }) {
  return (
    <div className="mx-auto max-w-2xl rounded-2xl border border-neutral-700/60 bg-[#111318] p-6 shadow-[0_8px_24px_rgba(0,0,0,.5)]">
      <Link href="/admin/usuarios" className="mb-5 inline-flex text-xs font-semibold text-[#FFC200]">← Volver a Usuarios</Link>
...
export function AdminUserEditorModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  return (
    <div role="dialog" aria-modal="true" aria-label="Editor de usuario" className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-neutral-700/60 bg-[#111318] p-6 shadow-[0_8px_24px_rgba(0,0,0,.5)]">
        <button type="button" aria-label="Cerrar editor" onClick={onClose} className="absolute right-4 top-4 z-10 cursor-pointer rounded-lg border border-neutral-700/60 bg-[#171A20] p-2 text-gray-400 hover:text-white"><X className="h-4 w-4" /></button>
