...
export default function MinecraftAdminView() {
  ...
  return (
    <div className="space-y-6">
       <div><p className="text-[10px] font-medium uppercase tracking-wider text-gray-500">Servidor</p><h1 className="mt-1 font-display text-2xl font-bold text-white">Vinculaciones de Minecraft</h1><p className="mt-2 text-xs font-semibold text-gray-400">La verificación y el acceso se activan automáticamente. Aquí puedes revisar, revocar o eliminar vinculaciones de Minecraft.</p></div>
      {error && <p className="rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-xs font-bold text-red-400">{error}</p>}
      {temporaryReset && <div className="rounded-2xl border border-amber-400/50 bg-amber-400/10 p-5 text-sm text-amber-100"><p className="font-bold">Contraseña temporal para {temporaryReset.username}</p><p className="mt-2 font-mono text-xl font-black tracking-wider">{temporaryReset.password}</p><p className="mt-2 text-xs text-amber-200">Entrégala por privado. Expira a las {new Date(temporaryReset.expiresAt).toLocaleTimeString('es-PE')}. El jugador debe cambiarla dentro de Minecraft.</p><button type="button" onClick={() => setTemporaryReset(null)} className="mt-3 rounded-lg border border-amber-300/40 px-3 py-2 text-xs font-bold text-amber-100">Ocultar contraseña</button></div>}
        {loading ? <p className="text-sm text-gray-500">Cargando...</p> : <div className="space-y-3">{requests.map((request) => <article key={request.id} className="rounded-2xl border border-neutral-700/60 bg-[#2b2d31] p-5 shadow-[0_4px_12px_rgba(0,0,0,.25)]"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-display text-lg font-bold text-white">{request.username} <span className="text-xs uppercase text-gray-500">{request.edition}</span></p><p className="mt-1 font-mono text-xs text-gray-500">{request.player_id}</p><p className={`mt-2 text-xs font-bold ${request.verified_at ? 'text-emerald-300' : 'text-amber-300'}`}>{requestStatus(request)}</p></div><div className="flex gap-2">{request.status !== 'revoked' && <button onClick={() => void resetPassword(request)} className="rounded-lg bg-amber-500/20 px-3 py-2 text-xs font-bold text-amber-200">Resetear contraseña</button>}{request.status === 'pending' && <button onClick={() => void update(request.id, 'reject')} className="rounded-lg bg-red-500/20 px-3 py-2 text-xs font-bold text-red-200">Rechazar</button>}{request.status === 'approved' && <button onClick={() => void update(request.id, 'revoke')} className="rounded-lg bg-red-500/20 px-3 py-2 text-xs font-bold text-red-200">Revocar</button>}<button onClick={() => void update(request.id, 'delete')} className="rounded-lg bg-red-700/30 px-3 py-2 text-xs font-bold text-red-200">Eliminar</button></div></div></article>)}</div>}
